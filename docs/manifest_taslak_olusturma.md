...
Bu not, yeni bir algoritma cekirdegini sisteme baglarken AI'nin nasil daha mekanik
calisacagini tarif eder.
...
- `ehcore.catalog.build_integration_checklist_text(...)`
- `ehcore.catalog.suggest_manifest_module(...)`
- `ehcore.catalog.manifest_draft_from_mapping(...)`
- `python -m ehcore.catalog suggest-module ...`
- `python -m ehcore.catalog render ...`
- `python -m ehcore.catalog check ...`

## Beklenen Akis

1. Algoritma dosyasi `src/ehcore/algorithms/**` altinda yazilir.
...
Kod yazmadan daha mekanik bir entegrasyon icin JSON spec + CLI yolu da kullanilabilir.
...
```json
{
  ...
  "algorithm_import_path": "ehcore.algorithms.detection.my_detector",
...
```bash
python -m ehcore.catalog suggest-module "Tespit" --node-id my_detector
```

Spec'ten manifest ve AI handoff cikarmak icin:

```bash
python -m ehcore.catalog render spec.json --output src/ehcore/catalog/manifests/detection.py --print-handoff
```

Spec'ten AI handoff ve entegrasyon checklist'i almak icin:

```bash
python -m ehcore.catalog check spec.json --print-module
```

## Ornek Taslak

```python
from ehcore.catalog import (
    ManifestConfigDraft,
    ManifestDraft,
    ManifestPortDraft,
...
Bu yardimcilar AI'nin isi tekrar tekrar ayni elle yazma kalibina dusmeden
yapabilmesi icin eklendi. Algoritma sahibi yine yalnizca algoritma dosyasina
dokunur; manifest ve UI baglama isi sistem tarafinda kalir.
