...
from ehplatform.contracts import check_port_compatibility
    ...
    if len(graph) == 0:
        messages.append(
            ValidationMessage(
                ...
                node_id="",
                message="Pipeline bos - en az bir blok ekleyin.",
            )
        )
        return False
    ...
    if not sources:
        messages.append(
            ValidationMessage(
                severity=Severity.ERROR,
                node_id="",
                message="Pipeline'da kaynak blok bulunamadi.",
            )
        )
    ...
    if not sinks:
        messages.append(
            ValidationMessage(
                ...
                node_id="",
                message="Pipeline'da goruntuleyici blok yok - cikti izlenemeyecek.",
            )
        )
    return True
    ...
    try:
        ...
        messages.append(
            ValidationMessage(
                severity=Severity.ERROR,
                node_id="",
                message=f"Sistem uzerinde dongu tespit edildi. Dahil bloklar: {', '.join(names)}",
            )
        )
        return False
    return True
        ...
        for err in validate_variable_usage(node.config, variables):
            ...
            messages.append(
                ValidationMessage(
                    severity=severity,
                    node_id=node.instance_id,
                    message=f"[{descriptor.display_name}] Degisken hatasi: {err}",
                )
            )
            ...
            if port.required and port.name not in connected_input_ports:
                ...
                messages.append(
                    ValidationMessage(
                        severity=severity,
                        node_id=node.instance_id,
                        message=f"[{descriptor.display_name}] Zorunlu giris '{port.display_name}' bagli degil.",
                    )
                )
        ...
        if src_port is None or tgt_port is None:
            messages.append(
                ValidationMessage(
                    ...
                    message=f"Gecersiz port adi: {edge.source_port} -> {edge.target_port}",
                )
            )
            continue
