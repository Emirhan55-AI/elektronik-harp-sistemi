...
from PySide6.QtWidgets import (
    ...
    QDockWidget, QFrame, QLabel, QHBoxLayout, QVBoxLayout,
    QToolButton, QMenu, QStyle
)

from app.theme.stylesheet import build_stylesheet
from app.theme.tokens import COLORS, SIZES
from app.strings import tr
from app.workspace import LAYOUT_PRESETS

# Canvas
from app.canvas.scene import NodeEditorScene
from app.canvas.view import NodeEditorView
from app.canvas.palette import BlockPalette

# Panels
from app.panels.inspector_panel import InspectorPanel
from app.panels.log_panel import LogPanel
from app.panels.event_feed_panel import EventFeedPanel
from app.panels.operation_summary_panel import OperationSummaryPanel

# Plotting
from app.plotting.spectrum_plot import SpectrumPlot
from app.plotting.waterfall_plot import WaterfallPlot
from app.plotting.detections_table import DetectionsTable

# Bridge
from app.bridge.app_controller import AppController

# Persistence
from app.persistence.project_io import save_project, load_project
...
class MainWindow(QMainWindow):
    ...
    def _create_menu_bar(self) -> None:
        ...
        select_all_act = QAction(tr.CTX_SELECT_ALL, self)
        select_all_act.setShortcut(QKeySequence.StandardKey.SelectAll)
        select_all_act.triggered.connect(self._select_all_scene_items)
        ...
        view_menu.addAction(self._system_dock.toggleViewAction())
    ...
    def _create_toolbar(self) -> None:
        ...
        host = QWidget()
        host.setObjectName("ToolbarHost")
        host_layout = QHBoxLayout(host)
        host_layout.setContentsMargins(10, 2, 10, 2)
        host_layout.setSpacing(6)

        self._run_menu = QMenu(self)
        self._run_menu_action = self._run_menu.addAction(tr.TOOLBAR_RUN)
        self._run_menu_action.triggered.connect(self._toggle_run_state)
        ...
        self._run_btn.setMenu(self._run_menu)
        self._run_btn.setIconSize(QSize(16, 16))
        self._run_btn.setAutoRaise(False)
        self._run_btn.setToolTip(tr.TOOLTIP_RUN)
        self._run_btn.clicked.connect(self._toggle_run_state)
        host_layout.addWidget(self._run_btn)

        self._reset_menu = QMenu(self)
        self._reset_menu_action = self._reset_menu.addAction(tr.TOOLBAR_RESET)
        self._reset_menu_action.triggered.connect(self._reset_pipeline)

        self._reset_btn = QToolButton()
        self._reset_btn.setObjectName("ActionButton")
        self._reset_btn.setPopupMode(QToolButton.ToolButtonPopupMode.MenuButtonPopup)
        self._reset_btn.setMenu(self._reset_menu)
        self._reset_btn.setIconSize(QSize(16, 16))
        self._reset_btn.setAutoRaise(False)
        self._reset_btn.setToolTip(tr.TOOLTIP_RESET)
        self._reset_btn.clicked.connect(self._reset_pipeline)
        self._reset_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_BrowserReload))
        host_layout.addWidget(self._reset_btn)

        self._design_mode_btn = QPushButton(tr.MODE_DESIGN)
        self._design_mode_btn.setObjectName("ModeToggle")
        self._design_mode_btn.setCheckable(True)
        self._design_mode_btn.clicked.connect(lambda: self._set_ui_mode("design"))
        host_layout.addWidget(self._design_mode_btn)

        self._operation_mode_btn = QPushButton(tr.MODE_OPERATION)
        self._operation_mode_btn.setObjectName("ModeToggle")
        self._operation_mode_btn.setCheckable(True)
        self._operation_mode_btn.clicked.connect(lambda: self._set_ui_mode("operation"))
        host_layout.addWidget(self._operation_mode_btn)
    ...
    def _create_central(self) -> None:
        ...
        self._palette_dock.setTitleBarWidget(self._create_hidden_title_bar())
        ...
        self._inspector_dock.setTitleBarWidget(self._create_hidden_title_bar())
        ...
        system_panel.setObjectName("SystemPanelRoot")
        system_layout = QVBoxLayout(system_panel)
        system_layout.setContentsMargins(0, 0, 0, 0)
        system_layout.setSpacing(0)

        system_header = QFrame()
        system_header.setObjectName("SystemPanelHeader")
        header_layout = QHBoxLayout(system_header)
        header_layout.setContentsMargins(0, 0, 0, 0)
        header_layout.setSpacing(0)

        tab_label = QLabel(tr.LOG_TITLE)
        tab_label.setObjectName("SystemPanelTabLabel")
        header_layout.addWidget(tab_label)
        header_layout.addStretch(1)

        clear_btn = QPushButton(tr.LOG_CLEAR)
        ...
        system_layout.addWidget(self._log_panel, 1)

        self._system_dock = QDockWidget(tr.LOG_TITLE, self)
        self._system_dock.setObjectName("SystemDock")
        self._system_dock.setWidget(system_panel)
        self._system_dock.setTitleBarWidget(self._create_hidden_title_bar())
        self.addDockWidget(Qt.DockWidgetArea.BottomDockWidgetArea, self._system_dock)
        self._log_dock = self._system_dock
    ...
    def _create_hidden_title_bar(self) -> QWidget:
        title_bar = QWidget()
        title_bar.setObjectName("HiddenDockTitle")
    ...
    def _connect_signals(self) -> None:
        ...
        self._properties.close_requested.connect(self._close_inspector_dock)
    ...
    def _close_inspector_dock(self) -> None:
        self._inspector_dock.hide()

    def _on_scene_selection_changed(self) -> None:
        if self._is_closing or not self._scene_is_alive():
            return
        try:
    ...
    def _set_ui_mode(self, mode: str, preserve_layout: bool = False) -> None:
        ...
        self.centralWidget().setVisible(mode == "design")
        self._view.setInteractive(mode == "design")
        self._scene.set_grid_visible(mode == "design")
    ...
    def _apply_layout_preset(self, preset_id: str, update_mode: bool = True) -> None:
        ...
        visibility = dict(preset.dock_visibility)
        if preset_id == "design":
            visibility.update(
                {
                    "palette": True,
                    "properties": self._inspector_dock.isVisible(),
                    "log": True,
                    "summary": False,
                    "spectrum": False,
                    "waterfall": False,
        ...
        for key, dock in dock_map.items():
            dock.setVisible(bool(visibility.get(key, False)))

        if preset_id == "design":
            self._palette_dock.raise_()
            self._system_dock.raise_()
        elif preset_id == "operation":
    ...
    def _run_pipeline(self) -> None:
        ...
        if errors:
            if self._ui_mode != "operation":
                self._system_dock.show()
                self._system_dock.raise_()
            self.statusBar().showMessage(
    ...
    def _on_pipeline_started(self) -> None:
        ...
        if self._ui_mode == "operation":
            self._apply_layout_preset("operation", update_mode=False)
        else:
            self._system_dock.show()
            self._system_dock.raise_()
    ...
    def _set_run_button_state(self, is_running: bool) -> None:
        ...
        self._run_btn.setToolTip(tr.TOOLTIP_STOP if is_running else tr.TOOLTIP_RUN)
        self._run_menu_action.setText(tr.TOOLBAR_STOP if is_running else tr.TOOLBAR_RUN)
        self._run_btn.setProperty("running", is_running)
