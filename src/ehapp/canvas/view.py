...
class NodeEditorView(QGraphicsView):
    ...
    def _update_rubber_band_style(self, current_pos: QPoint) -> None:
        ...
        if is_left_to_right:
            border = "rgba(96, 176, 255, 0.95)"
            fill = "rgba(96, 176, 255, 0.28)"
        else:
            border = "rgba(0, 230, 170, 0.95)"
            fill = "rgba(0, 230, 170, 0.22)"
        self._rubber_band.setStyleSheet(
            f"QRubberBand {{ border: 1px solid {border}; background-color: {fill}; }}"
        )
